"""
greedy_fp32_int8.py - Find optimal FP32/INT8 mixed configs for each
adjacent depth pair.

For each (deeper, shallower) pair: find the mix of FP32 and INT8 layers
on the deeper model that fits within the shallower model's FP32 size,
maximizing accuracy.

Algorithm:
  1. Start with all layers at FP32
  2. Greedily demote layers FP32 -> INT8, picking the layer with the
     smallest AP drop per byte saved
  3. Stop as soon as size <= target

Usage:
    python greedy_fp32_int8.py
    python greedy_fp32_int8.py --output greedy_fp32_int8_results.json
"""
...
# ── Greedy FP32->INT8 demotion ───────────────────────────────────────────────

def greedy_fp32_to_int8(depth, layer_params, non_layer_params, sensitivity_data,
                         target_size_bytes):
    """
    Start all FP32, greedily demote layers to INT8 (least sensitive first).
    Stop when size <= target.
    """
    layer_bits = {i: 32 for i in range(depth)}
    current_size = compute_size_bytes(layer_params, non_layer_params, layer_bits)

    allocation_log = [{
        "step": 0,
        "action": "start (all FP32)",
    ...
    if current_size <= target_size_bytes:
        allocation_log[0]["note"] = "Already at or below target"
    ...
    while current_size > target_size_bytes:
        ...
        for i in range(depth):
            ...
            # Bytes saved: FP32 -> INT8 = params * (4 - 1) = params * 3 bytes
            bytes_saved = layer_params[i] * 3  # 32bit=4bytes, 8bit=1byte

            # AP cost from sensitivity data (INT8 individual drop)
            key = f"int8_layer{i}"
            if key not in sensitivity_data:
                continue
            ap_drop = abs(sensitivity_data[key].get("ap_drop", float('inf')))
        ...
        ap_cost = abs(sensitivity_data[f"int8_layer{best_layer}"].get("ap_drop", 0))
        layer_bits[best_layer] = 8
        current_size -= best_savings
        total_ap_cost += ap_cost

        allocation_log.append({
            "step": step,
            "action": f"layer {best_layer}: FP32 -> INT8",
...
def main():
    ...
    parser.add_argument("--output", default="greedy_fp32_int8_results.json")
    ...
    for deeper, shallower in pairs:
        ...
        # Target: shallower model's FP32 size
        target_size_bytes = uniform[shallower_key]["fp32"]["size_bytes"]
        target_size_mb = target_size_bytes / (1024 * 1024)
        target_ap = uniform[shallower_key]["fp32"]["AP"]

        print(f"{'='*60}")
        print(f"PAIR: depth-{deeper} mixed FP32/INT8 -> depth-{shallower} FP32 size")
        ...
        fp32_size = compute_size_bytes(layer_params, non_layer_params,
                                        {i: 32 for i in range(deeper)})
        print(f"  Deeper FP32 size: {fp32_size/(1024*1024):.2f} MB")
        print(f"  Target size: {target_size_mb:.2f} MB")

        if fp32_size <= target_size_bytes:
            print(f"  Already fits at FP32! No quantization needed.")
            # Just evaluate FP32
            ...
            layer_bits = {i: 32 for i in range(deeper)}
            config_size = fp32_size
        else:
            # Run greedy
            sens_data = sensitivity[deeper_key]
            layer_bits, log = greedy_fp32_to_int8(
            ...
            for entry in log:
                if "layer_demoted" in entry:
                    print(f"    Step {entry['step']}: demote layer {entry['layer_demoted']} "
                          f"(AP cost: {entry['ap_cost_individual']:.6f}, "
                          f"size: {entry['size_mb']:.2f} MB)")
                else:
                    print(f"    {entry['action']} ({entry['size_mb']:.2f} MB)")

            config_size = compute_size_bytes(layer_params, non_layer_params, layer_bits)
            print(f"\n  Final config: {bits_str(layer_bits, deeper)}")
            print(f"  Final size: {config_size/(1024*1024):.2f} MB")

            # Evaluate
            print(f"  Evaluating...")
            t0 = time.time()
            model_q = apply_mixed_precision(model, layer_bits)
        ...
        config_bops = compute_total_bops(deeper, layer_bits)
        is_pareto = metrics["AP"] > target_ap
        ap_delta = metrics["AP"] - target_ap

        print(f"  Result: AP={metrics['AP']:.4f} vs target {target_ap:.4f} "
              f"(Δ={ap_delta:+.4f}) "
              f"{'✓ PARETO' if is_pareto else '✗ not Pareto'} ({elapsed:.1f}s)")

        num_int8 = sum(1 for v in layer_bits.values() if v == 8)
        num_fp32 = sum(1 for v in layer_bits.values() if v == 32)

        pair_key = f"depth{deeper}_vs_depth{shallower}_fp32"
        all_results[pair_key] = {
            "deeper_depth": deeper,
            "shallower_depth": shallower,
            "target_size_mb": target_size_mb,
            "target_ap": target_ap,
            "config": {str(k): v for k, v in layer_bits.items()},
            "config_str": bits_str(layer_bits, deeper),
            "num_fp32_layers": num_fp32,
            "num_int8_layers": num_int8,
            "actual_size_mb": config_size / (1024 * 1024),
            "actual_bops_gbops": config_bops / 1e9,
    ...
    # Summary
    print(f"\n{'='*60}")
    print("SUMMARY: Mixed FP32/INT8 vs Shallower FP32")
    print(f"{'='*60}")
    print(f"{'Pair':>25} {'Config':>25} {'Size MB':>9} "
          f"{'AP':>8} {'Target':>8} {'Delta':>8} {'Pareto':>7}")
    print("-" * 100)
    for key, r in all_results.items():
        p = "✓" if r["is_pareto_improvement"] else "✗"
        print(f"{key:>25} {r['config_str']:>25} {r['actual_size_mb']:>9.2f} "
              f"{r['metrics']['AP']:>8.4f} {r['target_ap']:>8.4f} "
              f"{r['ap_delta']:>+8.4f} {p:>7}")
